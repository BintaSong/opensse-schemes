...
#include "fastio/fastio_client_runner.hpp"
...
//             break;
    ...
    std::unique_ptr<sse::fastio::FastioClientRunner> client_runner;
    ...
    client_runner.reset( new sse::fastio::FastioClientRunner("localhost:4240", client_db, setup_size, n_keywords) );
...
//        auto post_callback = [&writer, &res_size, &writer_lock](index_type i)
        ...
        auto gen_callback = [&client_runner](const std::string &s, size_t i)
        {
            ...
            std::string index( reinterpret_cast<const char*>(&i), sse::fastio::kIndexSize );
