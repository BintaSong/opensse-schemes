...
// updated by Xiangfu Song for discoh, a hybird index based SSE

#include "discoh/discoh_client_runner.hpp"
...
int main(int argc, char** argv) {
    ...
    sse::logger::set_benchmark_file("benchmark_discoh_client.out");
...
//            input_files.push_back("/Volumes/Storage/WP_Inverted/inverted_index_all_sizes/inverted_index_10000.json");
    ...
    std::unique_ptr<sse::discoh::DiscohClientRunner> client_runner;
    ...
    client_runner.reset( new sse::discoh::DiscohClientRunner("localhost:4240", client_db, setup_size, n_keywords) );
...
//        auto post_callback = [&writer, &res_size, &writer_lock](index_type i)
    ...
    for (std::string &kw : keywords) {
        ...
        auto print_callback = [&logger_mtx, &log_stream, &first, &res_size](std::string res)
        {
            /* logger_mtx.lock();
