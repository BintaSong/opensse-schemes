...
class DiscohClientRunner { 
public:
    DiscohClientRunner(const std::string& address, const std::string& path, size_t setup_size = 1e5, uint32_t n_keywords = 1e4);
    ~DiscohClientRunner();
...
private:
    ...
    std::unique_ptr<discoh::Discoh::Stub> stub_;
    std::unique_ptr<DiscohClient> client_;
    
    struct {
        std::unique_ptr<grpc::ClientWriter<discoh::UpdateRequestMessage>> writer;
