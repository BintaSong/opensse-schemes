...
FastioImpl::FastioImpl(const std::string &path) : storage_path_(path), async_search_(true)
{
...
grpc::Status FastioImpl::setup(grpc::ServerContext *context,
                               const fastio::SetupMessage *message,
...
grpc::Status FastioImpl::search(grpc::ServerContext *context,
                                const fastio::SearchRequestMessage *mes,
...
grpc::Status FastioImpl::sync_search(grpc::ServerContext *context,
                                     const fastio::SearchRequestMessage *mes,
                                     grpc::ServerWriter<fastio::SearchReply> *writer)
{
    if (!server_)
...
grpc::Status FastioImpl::async_search(grpc::ServerContext *context,
                                      const fastio::SearchRequestMessage *mes,
                                      grpc::ServerWriter<fastio::SearchReply> *writer)
{
    if (!server_)
...
grpc::Status FastioImpl::update(grpc::ServerContext *context,
                                const fastio::UpdateRequestMessage *mes,
                                google::protobuf::Empty *e)
{
...
grpc::Status FastioImpl::bulk_update(grpc::ServerContext *context,
                                     grpc::ServerReader<fastio::UpdateRequestMessage> *reader, google::protobuf::Empty *e)
{
    if (!server_)
    {
        // problem, the server is already set up
    ...
    fastio::UpdateRequestMessage mes;
...
std::ostream &FastioImpl::print_stats(std::ostream &out) const
{
    if (server_)
    {
...
bool FastioImpl::search_asynchronously() const
{
    return async_search_;
}

void FastioImpl::set_search_asynchronously(bool flag)
{
    async_search_ = flag;
}
...
void run_fastio_server(const std::string &address, const std::string &server_db_path, grpc::Server **server_ptr, bool async_search)
{
    ...
    FastioImpl service(server_db_path);
