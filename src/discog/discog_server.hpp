...
public:
    ...
    DiscogServer(const std::string& db_path);
    DiscogServer(const std::string& db_path, const size_t tm_setup_size);
...
namespace sse {
    namespace discog {
        ...
        template <typename T>
        DiscogServer<T>::DiscogServer(const std::string& db_path) :
        edb_(db_path)
        {
        }
        
        template <typename T>
        DiscogServer<T>::DiscogServer(const std::string& db_path, const size_t tm_setup_size) :
        edb_(db_path)
        {
        ...
        template <typename T>
        bool DiscogServer<T>::get_unmask(const uint8_t *key, index_type &index, bool delete_key)
        {
        ...
        template <typename T>
        std::list<typename DiscogServer<T>::index_type> DiscogServer<T>::search(const SearchRequest& req, bool delete_results)
        {
        ...
        template <typename T>
        void DiscogServer<T>::search(const SearchRequest& req, const std::function<void(index_type)> &post_callback, bool delete_results)
        {
            
            if (logger::severity() <= logger::DBG) {
                logger::log(logger::DBG) << "[SERVER] Expected matches: " << req.add_count << std::endl;
                logger::log(logger::DBG) << "[SERVER] Number of search nodes: " << req.token_list.size() << std::endl;
            }
            ...
            auto get_callback = [this, &post_callback, delete_results](const uint8_t *key)
            {
                ...
                logger::log(logger::DBG) << "[SERVER] Derived global token: " << hex_string(std::string((const char*)key, TokenTree::kTokenSize)) << std::endl;

                bool found = false;
                crypto::Prf<TokenTree::kTokenSize> local_prf(key, TokenTree::kTokenSize);
                
                logger::log(logger::DBG) << "[SERVER] local_prf key data: " << hex_string(std::string((const char*)local_prf.key_data(), TokenTree::kTokenSize)) << std::endl;

                uint32_t local_counter = 0;
                do {
                    search_token_key_type local_st = local_prf.prf(std::to_string(local_counter)); // kSearchTokenKeySize == crypto::Prg::kKeySize
                    // logger::log(logger::DBG) << "[SERVER] INPUT: " << hex_string(std::string((const char*)key, TokenTree::kTokenSize)) << std::endl;
                    index_type index, mask;
                    update_token_type ut;
                    // index_type mask;
                    
                    if (logger::severity() <= logger::DBG) {
                        logger::log(logger::DBG) << "[SERVER] Derived local token: " << hex_string(local_st) << " for local counter "<< std::to_string(local_counter) << std::endl;
                       // logger::log(logger::DBG) << "[SERVER] Derived local token: " << hex_string(std::string((const char*)local_st.data(), TokenTree::kTokenSize)) << " for local counter "<< local_counter << std::endl;
                    }
                    // local_st = local_prf.prf(std::to_string(j)); // kSearchTokenKeySize == crypto::Prg::kKeySize
                    
                    gen_update_token_mask<T>(local_st, ut, mask);
                    
                    if (logger::severity() <= logger::DBG) {
                        //logger::log(logger::DBG) << "[SERVER] Derived token (ut) : " << hex_string(ut) << std::endl;
                        //logger::log(logger::DBG) << "[SERVER] Mask : " << hex_string(mask) << std::endl;
                    }
                    
                    found = retrieve_entry(ut, index, delete_results);
                    
                    if (found) {
                        if (logger::severity() <= logger::DBG) {
                            logger::log(logger::DBG) << "Found: " << hex_string(index) <<" for local counter: "<< local_counter << std::endl;
                        }
            ...
            for (auto it_token = req.token_list.begin(); it_token != req.token_list.end(); ++it_token) {
                ...
                if (logger::severity() <= logger::DBG) {
                    logger::log(logger::DBG) << "[SERVER] Search token key: " << hex_string(it_token->first) << std::endl;
                    logger::log(logger::DBG) << "[SERVER] Search token depth: " << std::dec << (uint32_t)(it_token->second) << std::endl;
                }
        ...
        template <typename T>
        void DiscogServer<T>::search_simple(const SearchRequest& req, const std::function<void(index_type)> &post_callback, bool delete_results)
        {       
            if (logger::severity() <= logger::DBG) {
                logger::log(logger::DBG) << "[SERVER] Expected matches: " << req.add_count << std::endl;
                logger::log(logger::DBG) << "[SERVER] Number of search nodes: " << req.token_list.size() << std::endl;
            ...
            for (auto it_token = req.token_list.begin(); it_token != req.token_list.end(); ++it_token) {
                ...
                for (uint64_t i = 0; i < count; i++) {
                    ...
                    if (logger::severity() <= logger::DBG) {
                        logger::log(logger::DBG) << "[SERVER] Derived leaf token (global secret): " << hex_string(t) << std::endl;
                    }
                    
                    bool found = false;
                    //auto local_prf = crypto::Prf<TokenTree::kTokenSize>(t);

                    uint32_t j = 0;
                    do { //find all level-2 index
                        auto local_prf = crypto::Prf<TokenTree::kTokenSize>(t);
                        search_token_key_type local_search_token = local_prf.prf(std::to_string(j));
                        //index_type index;
                        logger::log(logger::DBG) << "[SERVER] Derived local token: " << hex_string(local_search_token) << std::endl;

                        index_type r, mask;
                        update_token_type ut;

                        gen_update_token_mask(local_search_token, ut, mask);
                        
                        if (logger::severity() <= logger::DBG) {
                            logger::log(logger::DBG) << "[SERVER] Derived token (ut): " << hex_string(ut) << std::endl;
                            logger::log(logger::DBG) << "[SERVER] Mask : " << std::hex << mask << std::endl;
                        }
...
/*        
        template <typename T>
        std::list<typename DiscogServer<T>::index_type> DiscogServer<T>::search_simple_parallel(const SearchRequest& req, uint8_t threads_count, bool delete_results)
        {
        ...
        template <typename T>
        void DiscogServer<T>::search_simple_parallel(const SearchRequest& req, uint8_t threads_count, std::vector<index_type> &results, bool delete_results)
        {
        ...
        template <typename T>
        void DiscogServer<T>::search_simple_parallel(const SearchRequest& req, const std::function<void(index_type)> &post_callback, uint8_t threads_count, bool delete_results)
        {
        ...
        template <typename T>
        void DiscogServer<T>::search_simple_parallel(const SearchRequest& req, const std::function<void(index_type, uint8_t)> &post_callback, uint8_t threads_count, bool delete_results)
        {
            assert(threads_count>0);
            if(req.add_count == 0)
            {
...
*/        
        template <typename T>
        void DiscogServer<T>::update(const UpdateRequest<T>& req)
        {
            if (logger::severity() <= logger::DBG) {
                logger::log(logger::DBG) << "[SERVER] Update: (" << hex_string(req.token) << ", " << hex_string(req.index) << ")" << std::endl;
            }
    ...
    /*    template <typename T>
        std::ostream& DiscogServer<T>::print_stats(std::ostream& out) const
        {
            return out;
        }
    */    
        template <typename T>
        void DiscogServer<T>::flush_edb()
        {
            edb_.flush();
        }
    }
